    ...
    useEffect(() => {
        const filteredRecords = initialRecords.filter((item) => {
            return (
                ...
                (item.schoolPhoneNumber &&
                    item.schoolPhoneNumber
                        .toLowerCase()
                        .includes(search.toLowerCase())) ||
                (item.educationOffice &&
                    item.educationOffice.toLowerCase().includes(search.toLowerCase())) ||
                (item.schoolAddress &&
                    item.schoolAddress.toLowerCase().includes(search.toLowerCase())) ||
                (item.regDate &&
    ...
    const handleDeleteSchool = async (schoolId: number) => {
        if (window.confirm("정말로 이 학교를 삭제하시겠습니까?")) {
            try {
                ...
                setRecordsData(
                    formattedData.slice((page - 1) * pageSize, page * pageSize)
                );
            } catch (error) {
