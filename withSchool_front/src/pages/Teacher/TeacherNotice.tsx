...
const TeacherNotice = () => {
    ...
    const fetchNotices = async () => {
        try {
            ...
            const notices = await getClassNotices(Number(childId));
            console.log(notices);
    ...
    const selectMail = async (item: any) => {
        if (item) {
            if (item.type !== 'draft') {
                ...
                const detail = await getClassNoticeDetail(item.id);
                console.log(detail);
    ...
    const isHaveFile = async (fileId : any) => {
        try {
            const detail = await getClassNoticeDetail(fileId);
