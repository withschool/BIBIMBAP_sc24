...
import { subjectNotice, getSubjectNotices, subjectNoticeEdit, deleteSubjectNotice, getSubjectNoticeDetail } from '../../service/form';
...
const StudentSubjectNotice = () => {

    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(setPageTitle('과목 공지'));
    }, [dispatch]);
    ...
    const fetchNotices = async () => {
        try {
            const childId = localStorage.getItem('targetSubject');
            const notices = await getSubjectNotices(Number(childId));
            console.log(notices);
    ...
    const selectMail = async (item: any) => {
        if (item) {
            if (item.type !== 'draft') {
                ...
                const detail = await getSubjectNoticeDetail(item.id);
                console.log(detail);
    ...
    const isHaveFile = async (fileId : any) => {
        try {
            const detail = await getSubjectNoticeDetail(fileId);
...
export default StudentSubjectNotice;