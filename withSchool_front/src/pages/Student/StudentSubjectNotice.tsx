...
import { getClassNotice,getSubjectNoticeDetail, getSubjectNotices } from '../../service/form';
...
const SubjectNotice = () => {
    ...
    useEffect(() => {
        const fetchNoticeData = async () => {
            try {
                ...
                if (subjectId) {
                    const data = await getSubjectNotices(Number(subjectId));
                    setNoticeList(data);
                    console.log(data);
                } else {
                ...
                console.error('Error fetching subject notices:', error);
            }
        };
        fetchNoticeData();
    }, []);
    ...
    const handlePopUp = async (noticeId: number) => {
        try {
            const noticeDetail = await getSubjectNoticeDetail(noticeId);
                                ...
                                <div>
                                    ...
                                    <div className="p-4 relative">
                                        ...
                                        {(selectedNotice.filesURl) && (
                                            <div className="mt-8">
                                                <br />
                                                <div className="text-base mb-4">작성자</div>
                                                <div className="h-px border-b border-white-light dark:border-[#1b2e4b]"></div>
